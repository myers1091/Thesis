...
Electrons are identified using the ID and the LAr calorimeter. As an electron passes through matter, it produces Bremsstahlung radiation. These photons then convert back to electron-positron pairs and the process repeats. This shower of electrons, positrons, and photons is measured by the LAr calorimeter. To distinguish showers from electrons from those from photons, the ID is used. Since electrons are charged particles, they leave a track in the ID. This is not the case for photon.
\subsection{Muons}
...
\subsection{Jets}
Quarks very quickly undergo hadronization, with only the top quark decaying before hadronizing. This makes it impossible to measure a singular quark. Instead, collections of hadrons are grouped together into objects called jets. These objects are defined by various algorithms. Some examples are visualized in figure ~\ref{fig:jetalgo}. For this analysis, the AntiKt algorithm is used.\linebreak 
